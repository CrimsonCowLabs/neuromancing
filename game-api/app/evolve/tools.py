...
def walk_forward_windows(spec: dict, now: datetime) -> list[tuple[datetime, datetime]]:
    ...
    archive depth (so a 5m strategy is tested on real 5m history). Timeframes come from
    the shared rule — the one home for "which timeframes does this spec need"."""
    depth = min(_TF_DEPTH_DAYS.get(tf, 20) for tf in required_timeframes(spec))
