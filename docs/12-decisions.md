...
### D4 · Ollama Cloud by default, behind a pluggable OpenAI-compatible provider seam

**Why.** The proven pattern (from the `hl-froggy` system) is an **OpenAI-compatible endpoint** (`/v1`) through the `openai` SDK — our tool schema is already OpenAI function-calling shaped. Both the agent decision path (`agents/llm.py`) and the evolution reasoner (`evolve/llm.py`) resolve their transport (base_url / key / timeout / headers) + default model through **one config-driven seam** (`app/llm/provider.py`, `LLM_PROVIDER`) — the LLM analogue of the `Broker` seam (D3). **Ollama Cloud stays the default**; the open-source build can point at **OpenRouter**, a **local** server, or a **custom** endpoint without code changes. We keep **no native Anthropic client** — Anthropic is reachable *through OpenRouter* (which exposes it over the OpenAI-compatible API), so an Anthropic key never touches this code. The seam abstracts *transport*, not *capability*: tool-calls / structured output aren't uniformly supported across providers+models, and the deterministic fallback (D9) covers those failures.
**Hard-won details.** A **mandatory per-call timeout** (45s): hl-froggy once froze for 22 hours on a stuck call with no timeout. Timeout must stay under the decide-activity timeout so the LLM fails before Temporal does (no zombie call). And **read model names from config, never hardcode** — we shipped `deepseek-v3.2:cloud`, which had been *retired* (HTTP 410); every decision silently fell back until the fallback counters/logs (D9) surfaced it. Default is `deepseek-v4-flash:0731`. See [07](07-agent-brain.md).
